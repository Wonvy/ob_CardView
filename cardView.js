...
exports.CardView = exports.VIEW_TYPE_CARD = void 0;
const obsidian_1 = require("obsidian");
exports.VIEW_TYPE_CARD = 'card-view';
class CardView extends obsidian_1.ItemView {
    /**
     * 构造函数
     * @param leaf - 工作区叶子节点
     * @param plugin - 插件实例
    ...
    constructor(leaf, plugin) {
        ...
        this.recentFolders = [];
        this.cardSize = 280; // 默认卡片宽度
        this.MIN_CARD_SIZE = 280; // 最小卡片宽度
        this.MAX_CARD_SIZE = 600; // 最大卡片宽度
    ...
    loadNotes() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const cards = yield Promise.all(files.map(file => this.createNoteCard(file)));
            // 添加所有卡片到容器，并设置正确的宽度
            cards.forEach(card => {
    ...
    createNoteCard(file) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            // 添加点击事件
            folderPath.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
                ...
                if (fileExplorer) {
                    ...
                    if (fileExplorerView.expandFolder) {
                        yield this.revealFolderInExplorer(folder);
                        // 聚焦到文件浏览器
            ...
            try {
                ...
                // 渲染 Markdown 内容
                yield obsidian_1.MarkdownRenderer.renderMarkdown(content, noteContent, file.path, this);
                // 鼠标悬停事件
                card.addEventListener('mouseenter', () => __awaiter(this, void 0, void 0, function* () {
                    ...
                    try {
                        ...
                        yield obsidian_1.MarkdownRenderer.renderMarkdown(content, this.previewContainer, file.path, this);
                    }
                    catch (error) {
                ...
                // 修改事件监听
                openButton.addEventListener('click', (e) => __awaiter(this, void 0, void 0, function* () {
                    e.stopPropagation();
                    yield this.openInAppropriateLeaf(file);
                }));
                // 单击选择
                ...
                // 双击打开
                card.addEventListener('dblclick', () => __awaiter(this, void 0, void 0, function* () {
                    yield this.openInAppropriateLeaf(file);
                }));
                // 右键菜单
    ...
    filterByTag(tag) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const filteredFiles = files.filter(file => {
                var _a;
                const cache = this.app.metadataCache.getFileCache(file);
                return (_a = cache === null || cache === void 0 ? void 0 : cache.tags) === null || _a === void 0 ? void 0 : _a.some(t => t.tag === tag);
            });
            // 使用 Promise.all 等待所有卡片创建完成
            const cards = yield Promise.all(filteredFiles.map(file => this.createNoteCard(file)));
            // 添加所有卡片到容器
            cards.forEach(card => {
                this.container.appendChild(card);
            });
    ...
    revealFolderInExplorer(folder) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (fileExplorer) {
                ...
                if (folder === '根目录') {
                    if (fileExplorerView.expandFolder) {
                        yield fileExplorerView.expandFolder('/');
                    }
                    return;
                }
                // 展开并选中文件夹
                if (fileExplorerView.expandFolder) {
                    ...
                    for (const part of folderParts) {
                        ...
                        yield fileExplorerView.expandFolder(currentPath);
                    }
                    // 选中目标文件夹
                    if (fileExplorerView.setSelection) {
                        yield fileExplorerView.setSelection(folder);
                    }
                }
            }
        });
    ...
    createNewNote() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                // 创建新笔记
                const file = yield this.app.vault.create(`${fileName}.md`, '# ' + fileName + '\n\n');
                // 在新标签页中打开笔 记
            ...
            for (const date of sortedDates) {
                ...
                if (notes) {
                    for (const file of notes) {
                        ...
                        // 添加点击事件
                        noteItem.addEventListener('click', () => __awaiter(this, void 0, void 0, function* () {
                            const leaf = this.app.workspace.getLeaf('tab');
                            yield leaf.openFile(file);
                        }));
                        // 添加预览功能
                        noteItem.addEventListener('mouseenter', () => __awaiter(this, void 0, void 0, function* () {
                            try {
                                this.previewContainer.empty();
                                const content = yield this.app.vault.read(file);
                                yield obsidian_1.MarkdownRenderer.renderMarkdown(content, this.previewContainer, file.path, this);
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const filteredFiles = files.filter(file => {
                ...
                if (this.selectedTags.size > 0) {
                    const cache = this.app.metadataCache.getFileCache(file);
                    matchesTags = (_b = (_a = cache === null || cache === void 0 ? void 0 : cache.tags) === null || _a === void 0 ? void 0 : _a.some(t => this.selectedTags.has(t.tag))) !== null && _b !== void 0 ? _b : false;
                }
                // 日期过滤
            ...
            // 创建卡片并高亮搜索词
            const cards = yield Promise.all(filteredFiles.map(file => this.createNoteCard(file, this.currentSearchTerm)));
            cards.forEach(card => {
                if (card instanceof HTMLElement) {
                    card.style.width = `${this.cardSize}px`;
                    this.container.appendChild(card);
        ...
        if (files.length > 0) {
            ...
            menu.addItem((item) => {
                item
                    ...
                    this.revealFolderInExplorer(((_a = file.parent) === null || _a === void 0 ? void 0 : _a.path) || '/');
                });
            });
            menu.addItem((item) => {
                item
            ...
            menu.addItem((item) => {
                item
                    ...
                    .setIcon("trash")
                    .onClick(() => __awaiter(this, void 0, void 0, function* () {
                    const confirm = yield new ConfirmModal(this.app, "确认删除", `是否确定要删除选中的 ${files.length} 个文件？`).show();
                    if (confirm) {
                        for (const file of files) {
    ...
    showCalendar() {
        ...
        if (!this.calendarContainer) {
            ...
            (_a = contentSection.parentElement) === null || _a === void 0 ? void 0 : _a.insertBefore(this.calendarContainer, contentSection);
        }
...
// 添加文件选择模态框
class FileSelectionModal extends obsidian_1.Modal {
    ...
    moveFile(targetFolder) {
        return __awaiter(this, void 0, void 0, function* () {
            const newPath = `${targetFolder}/${this.file.name}`;
            yield this.app.fileManager.renameFile(this.file, newPath);
        });
    }
    onClose() {
        const { contentEl } = this;
...
// 修改增强的文件选择模态框
class EnhancedFileSelectionModal extends obsidian_1.Modal {
